...
| Field | Req | Notes |
|-------|-----|-------|
| `per_surface_artifacts` | yes | Table: per row, the surface name + its mock UI blob URL + its design instructions blob URL. URLs point at the orchestrator's sprint branch (`<orchestrator-root>/sprint-<N>/<surface-slug>.{html,md}`). |
